...
if (!class_exists('Redlight_Init')){
	...
	class Redlight_Init{
		 ...
		 * If you're building a theme based on redlight, use a find and replace
		 * to change 'redlight' to the name of your theme in all the template files.
		 */
			load_theme_textdomain( 'redlight', get_template_directory() . '/languages' );
			 ...
			 */
			function redlight_theme_add_editor_styles() {
				add_editor_style( 'custom-style.css' );
			}
			add_action( 'admin_init', 'redlight_theme_add_editor_styles' );
			...
			//Custom image size
			add_image_size('redlight-main-blog', 780, 450, true);
			add_image_size('redlight-blog', 336, 207, true);

			// This theme uses wp_nav_menu() in one location.
			register_nav_menus( array(
				'main-menu' => esc_html__( 'Primary Menu', 'redlight' ),
			) );
			...
			add_theme_support( 'custom-background', apply_filters( 'redlight_custom_background_args', array(
			...
			$GLOBALS['content_width'] = apply_filters( 'redlight_content_width', 640 );
		...
		public function widgets_init(){
			...
			register_sidebar( array(
				'name'          => esc_html__( 'Sidebar', 'redlight' ),
				'id'            => 'right-sidebar',
				'description'   => esc_html__( 'Add widgets here.', 'redlight' ),
			...
			$enable_footer_widgets = get_theme_mod('redlight_enable_footer_widgets', true);
			if($enable_footer_widgets == true){
				register_sidebar( array(
					'name'          => esc_html__( 'Footer Widget Area One', 'redlight' ),
					'id'            => 'footer-widget-1',
					'description'   => esc_html__( 'Add Footer  widgets here.', 'redlight' ),
				...
				register_sidebar( array(
					'name'          => esc_html__( 'Footer Widget Area Two', 'redlight' ),
					'id'            => 'footer-widget-2',
					'description'   => esc_html__( 'Add Footer widgets here.', 'redlight' ),
					'before_widget' => '<div id="%1$s" class="footer-widget widget %2$s">',
					'after_widget'  => '</div>',
				...
				register_sidebar( array(
					'name'          => esc_html__( 'Footer Widget Area Three', 'redlight' ),
					'id'            => 'footer-widget-3',
					'description'   => esc_html__( 'Add Footer widgets here.', 'redlight' ),
					'before_widget' => '<div id="%1$s" class="footer-widget widget %2$s">',
					'after_widget'  => '</div>',
				...
				register_sidebar( array(
					'name'          => esc_html__( 'Footer Widget Area Four', 'redlight' ),
					'id'            => 'footer-widget-4',
					'description'   => esc_html__( 'Add Footer widgets here.', 'redlight' ),
					'before_widget' => '<div id="%1$s" class="footer-widget widget %2$s">',
					'after_widget'  => '</div>',
			...
			register_sidebar( array(
				'name'          => esc_html__( 'Footer Menu Widget', 'redlight' ),
				'id'            => 'footer-menu',
				'description'   => esc_html__( 'Add Footer widgets here.', 'redlight' ),
		...
		public function include_files(){
			...
			$includes_files = array(
				...
				array(
					...
					'file-path' => REDLIGHT_INC
				),
			
				array(
					'file-name' => 'theme-inline-styles',
					'file-path' => REDLIGHT_THEME_STYLESHEETS
				),
			);
		...
		*redlight load font
		*/
	   	public static function  redlight_fonts_url() {
	        $font_url = '';
	        if ( 'off' !== esc_html_x( 'on', 'Google font: on or off', 'redlight' ) ) :
	            $font_url = add_query_arg(
	                array(
	                    'family' => urlencode( 'Raleway:300,400,500,600|Raleway:600,700&display=swap' ),
	                ), "//fonts.googleapis.com/css" );
	        endif;
	        return apply_filters( 'redlight_google_font_url', esc_url( $font_url ) );
	    }
		
		public function theme_css(){
			
			$ver = REDLIGHT_VERSION;
			$includes_css = array(

				array(
					'handle' => 'redlight-font',
					'src' => self::redlight_fonts_url(),
					'deps' => array(),
					'ver' => $ver,
					'media' => 'all'
				),
				...
				array(
					'handle' => 'bootstrap',
					'src' => REDLIGHT_CSS .'/bootstrap.min.css',
					'deps' => array(),
					'ver' => $ver,
					'media' => 'all'
				),
				...
				array(
					'handle' => 'font-awesome',
					'src' => REDLIGHT_CSS .'/font-awesome.min.css',
					'deps' => array(),
					'ver' => $ver,
					'media' => 'all'
				),
				array(
					'handle' => 'jquery-ui',
					'src' => REDLIGHT_CSS .'/jquery-ui.min.css',
					'deps' => array(),
					'ver' => $ver,
					'media' => 'all'
				),
				...
				array(
					...
					'src' => REDLIGHT_CSS .'/magnific-popup.css',
					'deps' => array(),
					'ver' => $ver,
					'media' => 'all'
				),
				array(
					'handle' => 'nice-select',
					'src' => REDLIGHT_CSS .'/nice-select.css',
					'deps' => array(),
					'ver' => $ver,
					'media' => 'all'
				),
			...
			wp_enqueue_style( 'redlight-style', get_stylesheet_uri());
		}
